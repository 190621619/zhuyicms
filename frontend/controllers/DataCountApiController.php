...
class DataCountApiController extends Controller {
     ...
     * 内部统计自动跳转
     */
    public function actionChannelCount() {
        $_returnUrl = $this->__getParams('return_url');
        //检验主分类
        $_mainTypeId = $this->__getParams('mtId');
        if (empty($_mainTypeId)) {
            $this->__error('mtId不能为空');
        ...
        if ($res) {
            if ($_returnUrl) {
                header('Location:' . urldecode($_returnUrl));                
            } else {
                $this->redirect([urldecode($_returnUrl)]);
            }
        } else {
            $this->redirect(['index']);
        }
